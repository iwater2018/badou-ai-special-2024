import numpy as np
import math
from utils.utils import nom_boxes
...
def generate_anchors(scales,ratios,shape,feature_stride,anchor_stride):
    ...
    shitfs_y = np.arange(0,shape[0],anchor_stride) * feature_stride
    shitfs_x = np.arange(0,shape[1],anchor_stride) * feature_stride
    shitfs_x, shitfs_y = np.meshgrid(shitfs_x,shitfs_y)

    box_widths, box_centers_x = np.meshgrid(widths,shitfs_x)
    box_heights,box_centers_y = np.meshgrid(heights,shitfs_y)
...
def get_anchors(config,image_shape):
    ...
    if not tuple(image_shape) in anchor_cache:
        ...
        anchor_cache[tuple(image_shape)] = nom_boxes(a,image_shape[:2])
