...
class PhotometricDistort(object):
    def __init__(self):
        self.pd = [
            RandomContrast(),  # 对比度随机
            ConvertColor(transform='HSV'),  # 转换颜色格式
            RandomSaturation(),  # 饱和度随机
            RandomHue(),  # 色调随机
...
class RandomBrightness(object):
    ...
    def __call__(self, image, boxes=None, labels=None):
        if random.randint(2):
            ...
            image = image.astype(np.float32)  # 转换为浮点数以进行计算
            image += delta
            image = np.clip(image, 0, 255)    # 裁剪像素值，确保在0到255的范围内
            image = image.astype(np.uint8)   # 转换回8位整数格式
        return image, boxes, labels
