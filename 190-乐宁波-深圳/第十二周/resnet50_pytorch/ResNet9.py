...
def resnet9(pretrained=False):
    ...
    return nn.Sequential(
        model,
        nn.Conv2d(512, 100, kernel_size=1),  # 1x1卷积层代替全连接层，输出100个类别
        nn.AdaptiveAvgPool2d((1, 1)),  # 全局平均池化，将每个通道的特征图压缩为1x1
        nn.Flatten()  # 压平为(batch_size, 100)
    )
