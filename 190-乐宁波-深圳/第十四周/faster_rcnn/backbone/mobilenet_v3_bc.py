...
class BlockE1(nn.Module):
    ...
    def __init__(self, kernel_size, in_size, expand_size, out_size, nolinear, semodule, stride):
        ...
        self.conv2 = nn.Conv2d(in_size, in_size, kernel_size=kernel_size, stride=stride, padding=kernel_size // 2,
...
class MobileNetV3BC(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList((
            _conv1,
            BlockE1(3, 24, 24, 24, nn.ReLU(inplace=True), None, 1),  ####112
            Block(3, 24, 72, 36, nn.ReLU(inplace=True), None, 2),  ####112
            Block(3, 36, 108, 36, nn.ReLU(inplace=True), None, 1),  ####56

            Block(3, 36, 108, 36, nn.ReLU(inplace=True), None, 1),  ####56
            Block(3, 36, 108, 36, nn.ReLU(inplace=True), None, 1),  ####56

            Block(5, 36, 108, 60, nn.ReLU(inplace=True), SeModule(108), 2),  ####56
            Block(5, 60, 180, 60, nn.ReLU(inplace=True), SeModule(180), 1),  ####28
            Block(5, 60, 180, 60, nn.ReLU(inplace=True), SeModule(180), 1),  ####28
            Block(3, 60, 240, 80, hswish(), None, 2),  ####28
            Block(3, 80, 200, 80, hswish(), None, 1),  ####14
            Block(3, 80, 184, 80, hswish(), None, 1),  ####14
            Block(3, 80, 184, 80, hswish(), None, 1),  ####14
            Block(3, 80, 480, 112, hswish(), SeModule(480), 1),  ####14
            Block(3, 112, 672, 112, hswish(), SeModule(672), 1),  ####14
            Block(5, 112, 672, 160, hswish(), SeModule(672), 2),  ####14
            Block(5, 160, 960, 160, hswish(), SeModule(960), 1),  ####7
            Block(5, 160, 960, 160, hswish(), SeModule(960), 1)  ####7
        ))
        ...
        if with_fc:
            ...
            if self.add_extra_stages == True:
                ...
                self.blocks.append(Block(5, 160, 960, 160, hswish(), SeModule(960), 1))  ####fifth stage for fpn
...
if __name__ == '__main__':
    # net = MobileNetV3BC(with_fc=True)
    # net.eval()
    ...
    net = MobileNetV3BC(with_fc=True).cuda()
