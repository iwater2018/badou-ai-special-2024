...
class BBoxUtility(object):
    def __init__(self, priors=None, overlap_threshold=0.7, ignore_threshold=0.3, nms_thresh=0.7, top_k=300):
        self.proirs = priors
        self.nums_priors = 0 if priors is None else len(priors)
        self.overlap_thresold = overlap_threshold
    ...
    def iou(self, box):
        #计算出每个真实框与所有先验框的iou
        #判断真实框与先验框的重合情况
        inter_upleft = np.maximum(self.proirs[:, :2], box[:2])
        inter_botright = np.minimum(self.proirs[:, 2:4], box[2:])
        ...
        #先验框的面积
        area_gt = (self.proirs[:, 2] - self.proirs[:, 0]) * (self.proirs[:, 3] - self.proirs[:, 1])
        #计算iou
    ...
    def encode_box(self, box, return_iou=True):
        iou = self.iou(box)
        encoded_box = np.zeros((self.nums_priors, 4 + return_iou))

        # 找到每一个真实框，重合程度较高的先验框
        assign_mask = iou > self.overlap_thresold
        #any()函数用于判断一个可迭代对象中的元素是否有至少一个为真
        if not assign_mask.any():
            assign_mask[iou.argmax()] = True #argmax()求数组最大值的索引
        if return_iou:
            encoded_box[:, -1][assign_mask] = iou[assign_mask]

        #找到对应先验框
        assigned_priors = self.proirs[assign_mask]
        ...
        return encoded_box.ravel() #将多维数组展平为一维数组
