...
class yolo:
    ...
    def _batch_normalization_layer(self, input_layer, name=None, training=True,  norm_decay=0.99, norm_epsilon=1e-3):
        ...
        ----------
            ...
            momentum: 默认是0.99，这个参数就是滑动平均的β值
            epsilon: 默认是1e-3，标准化公式里面的ɛ参数
            name: batchnorm层的名字
            trainging: 是否为训练过程
            norm_decay: 在预测时计算moving average时的衰减率
            norm_epsilon: 方差加上极小的数，防止除以0的情况
        ...
        '''
        #设置training参数为True,需要逐个神经元逐个样本地来计算，这个batch在某一层输出的均值和标准差，然后再对该层的输出进行标准化。同时还要学习gamma和beta两个参数。
    ...
    def _conv2d_layer(self, inputs, filters_num, kernel_size, name, use_bias=False, strides=1):
        ...
        """
        #kernel_initializer对该层中的权值进行正则化，亦即对权值进行限制，使其不至于过大
    ...
    def _darknet53(self, inputs, conv_index, training=True, norm_decay=0.99, norm_epsilon = 1e-3):
        ...
        with tf.variable_scope('darknet53'):
            ...
            conv = self._conv2d_layer(inputs, filters_num=32, kernel_size=3, name="conv2d_" + str(conv_index))
    ...
    #---------------------------------------#
    def _yolov3_block(self, inputs, filters_num, out_filters, conv_index, training = True, norm_decay = 0.99, norm_epsilon = 1e-3):
        """
        Introduction
        ------------
    ...
    def yolo_inference(self, inputs, num_anchors, num_classes, training=True):
        ...
        conv2d_size_52, conv2d_size_26, conv2d_size_13, conv_index = self._darknet53(inputs, conv_index, training=training, norm_decay=self.norm_decay, norm_epsilon=self.norm_epsilon)
        print(conv2d_size_52.shape, conv2d_size_26.shape, conv2d_size_13.shape, conv_index)
        with tf.variable_scope('yolo'):
            # --------------------------------------#
            #   获得第一个特征层 conv2d_59
            # --------------------------------------#
            # conv2d_13 = 13,13,512，conv2d_59 = 13,13,255(3x(80+5))
            conv2d_57, conv2d_59, conv_index = self._yolov3_block(conv2d_size_13, 512, num_anchors * (num_classes + 5), conv_index=conv_index, training=training, norm_decay=self.norm_decay, norm_epsilon=self.norm_epsilon)
            print(conv2d_57.shape, conv2d_59.shape, conv_index)
            # --------------------------------------#
            #   获得第二个特征层  conv2d_69
            # --------------------------------------#
            conv2d_60 = self._conv2d_layer(conv2d_57, filters_num=256, kernel_size=1, strides=1, name="conv2d_" + str(conv_index))
            conv2d_60 = self._batch_normalization_layer(conv2d_60, name="batch_normalization_"  + str(conv_index), norm_decay=self.norm_decay, norm_epsilon=self.norm_epsilon)
            conv_index += 1
            #upsample = 26,26,256
            upSample_0 = tf.image.resize_nearest_neighbor(conv2d_60, [2 * tf.shape(conv2d_60)[1], 2 * tf.shape(conv2d_60)[1]], name='upSample_0')
            #route_size_26 = (26,26,768)
            route_size_26 = tf.concat([upSample_0, conv2d_size_26], axis=-1, name='route_size_26')
            # conv2d_65 = 52,52,256，conv2d_67 = 26,26,255
            conv2d_65, conv2d_69, conv_index = self._yolov3_block(route_size_26, 256, num_anchors * (num_classes + 5), conv_index=conv_index, training=training, norm_decay=self.norm_decay, norm_epsilon=self.norm_epsilon)

            # --------------------------------------#
            #   获得第三个特征层 conv2d_75
            # --------------------------------------#
            conv2d_68 = self._conv2d_layer(conv2d_65, filters_num=128, kernel_size=1, name="conv2d_" + str(conv_index))
            ...
            # route1= 52,52,384
            route_size_52 = tf.concat([upSample_1, conv2d_size_52], axis=-1, name='route_size_52')
            _, conv2d_75, _ = self._yolov3_block(route_size_52, 128, num_anchors * (num_classes + 5), conv_index, training=training, norm_decay=self.norm_decay, norm_epsilon=self.norm_epsilon)
        return [conv2d_59, conv2d_69, conv2d_75]
    ...
    #         conv2d_57, conv2d_59, conv_index = self._yolov3_block(conv, 512, num_anchors * (num_classes + 5), conv_index = conv_index, training = training, norm_decay = self.norm_decay, norm_epsilon = self.norm_epsilon)
    #
    #         #--------------------------------------#
    #         #   获得第二个特征层
    #         #--------------------------------------#
    ...
    #         # conv2d_65 = 52,52,256，conv2d_67 = 26,26,255
    #         conv2d_65, conv2d_67, conv_index = self._yolov3_block(route0, 256, num_anchors * (num_classes + 5), conv_index = conv_index, training = training, norm_decay = self.norm_decay, norm_epsilon = self.norm_epsilon)
    #
    #         #--------------------------------------#
    #         #   获得第三个特征层
    #         #--------------------------------------#
    ...
    #         _, conv2d_75, _ = self._yolov3_block(route1, 128, num_anchors * (num_classes + 5), conv_index = conv_index, training = training, norm_decay = self.norm_decay, norm_epsilon = self.norm_epsilon)
