...
from model.yolov3_model import yolo
...
class yolo_predictor:
    ...
    def correct_boxes(self, box_xy, box_wh, input_shape, image_shape):
        ...
        #tf.round四舍五入，计算一个张量的各个维度上元素的最小值
    ...
    def eval(self, yolo_outputs, image_shape, max_boxes=20):
        ...
        for i in range(len(yolo_outputs)):
            _boxes, _box_scores = self.boxes_and_scores(yolo_outputs[i], self.anchors[anchors_mask[i]], len(self.class_names), input_shape, image_shape)
            boxes.append(_boxes)
